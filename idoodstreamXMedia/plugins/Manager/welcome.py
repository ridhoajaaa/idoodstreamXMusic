...
from idoodstreamXMedia.mongo.welcomedb import is_on, set_state, bump, cool, auto_on

BG_PATH = "idoodstreamXMedia/assets/idoodstream/welcome.png"
FALLBACK_PIC = "idoodstreamXMedia/assets/upic.png"
FONT_PATH = "idoodstreamXMedia/assets/idoodstream/Arimo.ttf"
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    async def cleanup(path):
        if path and os.path.exists(path) and not os.path.abspath(path).startswith(os.path.abspath("idoodstreamXMedia/assets")):
            try:
                os.remove(path)
            except:
                pass
